...
describe('RemindersPage — claim step', () => {
  ...
  it('no match + no users doc → the full-screen "not registered" takeover shows, not a plain banner', async () => {
    ...
    expect(screen.getByText(/Ask your doctor about UMC/)).toBeInTheDocument()
    expect(screen.queryByText(/as a non-patient/)).not.toBeInTheDocument()
  ...
  it('no match + existing non-patient account → the takeover, saying it is a non-patient number; signed out, never deleted', async () => {
    vi.mocked(claimActions.previewClaim).mockResolvedValue({ found: false })
    vi.mocked(claimActions.usersDocExists).mockResolvedValue(true)
    renderWith({ status: 'signed-in', user: patientB, profile: { role: 'doctor' } })
    expect(await screen.findByText('Uh oh!')).toBeInTheDocument()
    expect(screen.getByText('This number is already registered in the UMC app as a non-patient.')).toBeInTheDocument()
    expect(screen.queryByText(/Ask your doctor/)).not.toBeInTheDocument()
    expect(screen.queryByText(/already has a UMC account/)).not.toBeInTheDocument()
    await waitFor(() => expect(claimActions.signOutExisting).toHaveBeenCalledTimes(1))
    expect(claimActions.deleteOrphanAccount).not.toHaveBeenCalled()
  })
  it('no match + existing patient account → still the plain banner, not the takeover', async () => {
    vi.mocked(claimActions.previewClaim).mockResolvedValue({ found: false })
    vi.mocked(claimActions.usersDocExists).mockResolvedValue(true)
    renderWith({ status: 'signed-in', user: patientB, profile: { role: 'patient' } })
    await waitFor(() => expect(claimActions.signOutExisting).toHaveBeenCalledTimes(1))
